...
void PlotCandlestick(const char* label_id, const double* xs, const double* opens, const double* closes, const double* lows, const double* highs, int count, bool tooltip, float width_percent, ImVec4 bullCol, ImVec4 bearCol) {
	...
	if (ImPlot::IsPlotHovered() && tooltip) {
		...
		if (idx != -1) {
			...
			ImGui::Text("Open:  %.2f ELO", opens[idx]);
			ImGui::Text("Close: %.2f ELO", closes[idx]);
			ImGui::Text("Low:   %.2f ELO", lows[idx]);
			ImGui::Text("High:  %.2f ELO", highs[idx]);
			ImGui::EndTooltip();
		}
	}
	...
	void ProfilePanel::OnImGuiRender()
	{
		...
		if (ImGui::BeginTable("table", 2, ImGuiTableFlags_BordersInnerV | ImGuiTableFlags_Resizable))
		{
			...
			if (ImGui::BeginTabItem("Statistics"))
			{
				...
				static int data1[] = { 10,  20,  30, 40};
				static ImPlotPieChartFlags flags = 0;
				ImGui::SetNextItemWidth(250);
				ImGui::DragInt4("Values", data1, 5, 0, 100);

				if (ImPlot::BeginPlot("##Pie1", ImVec2(500, 400), ImPlotFlags_Equal | ImPlotFlags_NoMouseText | ImPlotFlags_NoInputs))
				{
					ImPlot::SetupLegend(ImPlotLocation_East, ImPlotLegendFlags_Outside);
					ImPlot::SetupAxes(nullptr, nullptr, ImPlotAxisFlags_NoDecorations, ImPlotAxisFlags_NoDecorations);
					ImPlot::SetupAxesLimits(0, 1, 0, 1);
					ImPlot::PlotPieChart(labels1, data1, 4, 0.5, 0.5, 0.4, "%.0f", 300, flags);
					ImPlot::EndPlot();
				}
				...
				if (ImPlot::BeginPlot("Candlestick Chart", ImVec2(-1, 0))) {
					...
					PlotCandlestick("LiChess ELO", dates, opens, closes, lows, highs, 218, tooltip, 0.25f, bullCol, bearCol);
					ImPlot::EndPlot();
				}
